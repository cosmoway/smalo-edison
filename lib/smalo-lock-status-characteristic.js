...
function SmaloLockStatusCharacteristic(smalo) {
  ...
  this.smalo = smalo;
  this._value = new Buffer(this.smalo.doorStatus.getStatus());
  this._updateValueCallback = null;
  var self = this;

  debug('LockStatus init. ' + this.smalo.lockStatus);

  this.smalo.doorStatus.on('changeStatus', function(lockStatus){
    // lockStatus = locked or unlocked or unknown
...
SmaloLockStatusCharacteristic.prototype.onReadRequest = function(offset, callback) {
  console.info('[Read] LockStatus Request offset: ' + offset);
